...
async function runTranslate() {
    ...
    if(isComp) {
        ...
        Promise.all([
            fetchSingle(txt, 'auto', tgt, 'google_gtx'),
            fetchSingle(txt, 'auto', tgt, 'mymemory'),
            fetchSingle(txt, 'auto', tgt, 'google_dict')
        ]).then(results => {
            document.getElementById('outComp1').value = results[0] || "Server Sibuk";
            document.getElementById('outComp2').value = results[1] || "Limit Harian";
...
async function fetchSingle(text, s, t, type) {
    ...
    try {
        ...
        if (type === 'google_gtx') {
            ...
            let j = await r.json(); res = j[0].map(x => x[0]).join('');
        }
            ...
            let r = await fetch(url, { signal: controller.signal });
            let j = await r.json(); res = j.translation;
        }
            ...
            let r = await fetch("https://translate.argosopentech.com/translate", { method: 'POST', body: JSON.stringify({ q: text, source: s, target: t, format: 'text' }), headers: { 'Content-Type': 'application/json' }, signal: controller.signal });
            let j = await r.json(); res = j.translatedText;
        }
